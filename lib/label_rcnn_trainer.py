...
class LRCNNTrainer(Trainer):
  ...
  def draw_gt_pred_box(self,y_pred,image):
  ...
  def eval_action(self,x_single,y_single,step,logger):
    y_pred = self.model(x_single)
    bx_img = self.draw_gt_pred_box(y_pred, x_single)
    gt_box = tf.reshape(y_single[:,1:],[1,]+y_single[:,1:].shape[-2:])
    gt_box_img = tf.image.draw_bounding_boxes(x_single,gt_box,tf.convert_to_tensor([[1.0,0.0,0.0]]))
