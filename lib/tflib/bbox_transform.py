...
def bbox_transform_inv_tf(boxes, deltas, im_info):
  ...
  pred_boxes0 = tf.clip_by_value(tf.subtract(pred_ctr_x, pred_w * 0.5), 0, im_info[1] - 1)
  pred_boxes1 = tf.clip_by_value(tf.subtract(pred_ctr_y, pred_h * 0.5), 0, im_info[0] - 1)
  pred_boxes2 = tf.clip_by_value(tf.add(pred_ctr_x, pred_w * 0.5), 0, im_info[1] - 1)
  pred_boxes3 = tf.clip_by_value(tf.add(pred_ctr_y, pred_h * 0.5), 0, im_info[0] - 1)
...
def clip_boxes_tf(boxes, im_info):
  ...
  return tf.stack([b13[:,0], b02[:,0], b13[:,1], b02[:,1]], axis=1)